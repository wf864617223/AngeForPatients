...
public class CalendarPickerView extends ListView {
     ...
     *                     {@code minDate}.
     */
    public void init(Date selectedDate, Date minDate, Date maxDate, List<HashMap<String, String>> myList) {
        this.myList = myList;
    ...
    private class CellClickedListener implements MonthView.Listener {
        public void handleClick(MonthCellDescriptor cell) {
            if (!betweenDates(cell.getDate(), minCal, maxCal)) {
                ...
                String selectDate = sdf.format(cell.getDate());
                for (int i = 0; i < myList.size(); i++) {
                    if (selectDate.equals(myList.get(i).get("date"))) {
                        if ("true".equals(myList.get(i).get("isFull"))) {
                            Toast.makeText(getContext(), "预约人数已满，请重新选择预约日期", Toast.LENGTH_SHORT).show();
                            return;
                        } else {
//                            Toast.makeText(getContext(), "已选择预约时间", Toast.LENGTH_SHORT).show();
                ...
                Toast.makeText(getContext(), "抱歉，该日期暂未排班，请重新选择", Toast.LENGTH_SHORT).show();
            }
        }
    }
