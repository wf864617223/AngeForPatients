...
//        dbHelper = DbOpenHelper.getInstance(getHxInstance());
//    }
    private DemoDBManager() {
        dbHelper = DbOpenHelper.getInstance(new HxApplication().getInstance().getApplicationContext());
    }
     ...
     * @return
     */
    private Context getHxInstance() {
        if (hxApplication == null) {
            return new HxApplication().getInstance().getApplicationContext();
        } else {
